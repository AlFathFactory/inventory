...
type LowStockTableProps = { rows: LowStockRow[]; isLoading: boolean; pagination: { paginatedItems: LowStockRow[]; currentPage: number; pageSize: number; totalItems: number; totalPages: number; pageStart: number; pageEnd: number; setCurrentPage: (page: number) => void; setPageSize: (size: number) => void }; onItemClick?: (row: LowStockRow) => void }
...
export function LowStockTable({ rows, isLoading, pagination, onItemClick }: LowStockTableProps) {
  ...
  return <div className="overflow-hidden rounded-[24px] border border-[var(--app-border)]"><DataTable columns={tableColumns} rows={pagination.paginatedItems} getRowKey={(row) => row.id} stickyHeader maxHeightClassName="max-h-[70vh] overflow-auto" tableClassName="divide-y divide-slate-200 bg-white" rowClassName="hover:bg-slate-50" onRowClick={onItemClick} /><TablePagination currentPage={pagination.currentPage} pageSize={pagination.pageSize} totalItems={pagination.totalItems} totalPages={pagination.totalPages} pageStart={pagination.pageStart} pageEnd={pagination.pageEnd} onPageChange={pagination.setCurrentPage} onPageSizeChange={pagination.setPageSize} /></div>
}
