...
export function generateLowStockReport(rows: LowStockRow[], showExpiryDate: boolean) {
  ...
  const headers = ['م', 'الحالة', 'المخزن', 'الصنف', 'القسم', ...(showExpiryDate ? ['تاريخ الانتهاء'] : []), 'الرصيد الحالي', 'الحد الأدنى']
  const reportRows = rows.map((row, index) => [String(index + 1), getAlertStatusLabel(row.status), row.categoryLabel, row.itemName, row.projectName ?? '—', ...(showExpiryDate ? [row.expiryDateLabel] : []), formatNumber(row.stockBalance), formatNumber(row.minQuantity)])
