      ...
      <table className="min-w-full text-right">
        <thead className="bg-slate-50 text-sm font-semibold text-slate-800">
          <tr>
            ...
            <th className="px-6 py-5">الحد الأدنى</th>
            <th className="px-6 py-5">الحالة</th>
            <th className="px-6 py-5">إجراء</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
          {rows.map((row) => (
            <tr key={row.id}>
              ...
              <td className="px-6 py-5">{row.stockBalance}</td>
              <td className="px-6 py-5">{row.minQuantity}</td>
              ...
              <td className="px-6 py-5 text-slate-700">{row.actionLabel}</td>
            </tr>
          ))}
        </tbody>
      </table>
