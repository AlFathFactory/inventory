create index if not exists inventory_operations_reports_idx
  ...
  include (operation_type, quantity, item_name)
  where operation_type in ('add', 'issue');
...
revoke all on function public.get_aggregated_inventory_report_rpc(date,date,text,text,text,integer,integer)
  from public;
grant execute on function public.get_aggregated_inventory_report_rpc(date,date,text,text,text,integer,integer)
