create or replace function public.normalize_inventory_search_term(value text)
returns text
language sql
immutable
parallel safe
  ...
  select pg_catalog.regexp_replace(
    pg_catalog.translate(
      pg_catalog.regexp_replace(
        pg_catalog.regexp_replace(
          normalize(pg_catalog.lower(value), NFKD),
          '[ً-ٰٟـ]+',
          '',
          'g'
        ),
...
revoke all on function public.normalize_inventory_search_term(text) from public;
grant execute on function public.normalize_inventory_search_term(text) to anon, authenticated;
...
as $$
  with search_input as (
    select nullif(public.normalize_inventory_search_term(p_search), '') as value
  ),
  filtered_operations as materialized (
    ...
    where operation.operation_type in ('add', 'issue')
      and (nullif(btrim(p_operation_type), '') is null or operation.operation_type = p_operation_type)
      ...
      and (nullif(btrim(p_category_name), '') is null or coalesce(operation.category_name, operation.category_label) = p_category_name)
      and (nullif(btrim(p_project_name), '') is null or coalesce(operation.project_name, operation.project) = p_project_name)
      and (
        search_input.value is null
        or pg_catalog.strpos(
          public.normalize_inventory_search_term(
            concat_ws(
              ' ', operation.item_name, operation.item_label, operation.category_name,
              operation.category_label, operation.project_name, operation.project, operation.item_code
            )
          ),
          search_input.value
        ) > 0
          ...
          and exists (
            ...
            where searched_material.id = operation.item_id
              and pg_catalog.strpos(
                public.normalize_inventory_search_term(searched_material.code_number),
                search_input.value
              ) > 0
          )
        )
  ...
  aggregated_items as materialized (
    select
      ...
      max(operation.operation_date) as last_operation_date,
    ...
    left join public.raw_materials as raw_material
      on operation.table_name = 'raw_materials' and raw_material.id = operation.item_id
    group by operation.table_name, operation.item_id
  ),
  paged_items as (
    select *
    from aggregated_items
    order by last_operation_date desc nulls last, lower(item_name), table_name, item_id
  ...
  select jsonb_build_object(
    'rows', coalesce(
      (select jsonb_agg(to_jsonb(item) order by item.last_operation_date desc nulls last, lower(item.item_name), item.table_name, item.item_id) from paged_items as item),
      '[]'::jsonb
    ),
...
revoke all on function public.get_aggregated_inventory_report_rpc(date,date,text,text,text,text,integer,integer) from public;
grant execute on function public.get_aggregated_inventory_report_rpc(date,date,text,text,text,text,integer,integer) to anon, authenticated;
